...
// Derive the extension directory from this module's URL.
// import.meta.url is "file:///path/to/extension/indicator.js"
const _extensionPath = import.meta.url.replace('file://', '').replace(/\/[^/]+$/, '');
...
export const MultiMonitorsIndicator = (() => {
    let MultiMonitorsIndicator = class MultiMonitorsIndicator extends PanelMenu.Button {
        ...
        _syncIndicatorsVisible() {
            this._mmStatusIcon.visible =
                this._mmStatusIcon.get_children().some(a => a.visible);
        }

        _icon_name(icon, iconName) {
            icon.set_gicon(
                Gio.icon_new_for_string(
                    `${_extensionPath}/icons/${iconName}.svg`));
        }
